...
6. 教练：客户详情 → 逐天查看 → 看「审阅时留意」→ 某天不合适就「改这一天」（换一个动作、改一样食物的克数，保存）→ 确认发布（大问题才写原因打回，Wendy 重跑）。
7. 测试客户：今天页出现计划 → 打卡弹正 → 身体反馈选「有地方疼」→ 看到停止提示。
8. 教练：工作台出现身体提醒 →「已沟通」→ 客户详情「调整计划」，从明天起把让 TA 疼的动作换掉 → 确认调整；测试客户明天那天看到新动作、今天不变。
9. 教练：写一句「给 TA 的一句话」；测试客户今天页「这一周」卡片出现「教练说」。
10. 教练：给测试客户写备注、归档再移回，确认客户那边什么都看不到。
