...
- 食物 60 种，六大类各至少 4 种；热量以 USDA FoodData Central SR Legacy 为主（公共领域，按 fdcId 回查），馒头、小米粥用《中国食物成分表》。原有食物的热量随来源统一而更新（如水煮蛋 143 → 155 kcal/100g）。
...
- 《中国食物成分表》查询平台声明未经许可不得商用；目前只用了两项（馒头、小米粥），正式商用前要么取得授权，要么换源。
